...
export function renderBlobid(genes: Genotype, time: number, seed: number): PixelGrid {
  ...
  // Dome — hooks hoisted out of the per-pixel loop to avoid allocations
  const domeHooks = {
    stripe: (_dx: number, dy: number) => Math.abs(dy) % 3 === 0,
    highlight: (dx: number, dy: number) => dy < -hh * 0.5 && Math.abs(dx) < hw * 0.3,
    noiseThreshold: 0.8,
  };
  for (let dy = -hh - 1; dy <= 1; dy++) {
    for (let dx = -hw - 1; dx <= hw + 1; dx++) {
      ...
      if (d <= 1.0) {
        setPixel(grid, cx + dx, cy + dy, bodyPixelColor(style, dx, dy, d > 0.85, domeHooks));
      }
    }
  }
